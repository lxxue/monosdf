...
class SceneDatasetDN(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        image_paths = glob_data(os.path.join('{0}'.format(self.instance_dir), "image", "*.png"))
        depth_paths = glob_data(os.path.join('{0}'.format(self.instance_dir), "depth", "*.npy"))
        normal_paths = glob_data(os.path.join('{0}'.format(self.instance_dir), "normal", "*.npy"))
        
        # mask is only used in the replica dataset as some monocular depth predictions have very large error and we ignore it
        if use_mask:
            mask_paths = glob_data(os.path.join('{0}'.format(self.instance_dir), "mask", "*.png"))
        else:
            mask_paths = None

        print(len(image_paths), len(depth_paths), len(normal_paths), len(mask_paths))
